(() => {
  ...
  function updateVersionText(){
    document.title = document.title.replace(/Ver\s*2\.36\.1/g, `Ver${VERSION}`);
    $$('.splashVersion,.printBrandSub').forEach(el => { if(/2\.36\.1/.test(el.textContent)) el.textContent = el.textContent.replace(/2\.36\.1/g,VERSION); });
  }
